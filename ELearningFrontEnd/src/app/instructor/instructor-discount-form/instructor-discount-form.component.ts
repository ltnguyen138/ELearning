...
@Component({
  selector: 'app-instructor-discount-form',
  templateUrl: './instructor-discount-form.component.html',
  styleUrls: ['./instructor-discount-form.component.css']
})
export class InstructorDiscountFormComponent implements OnInit {
    ...
    saveDiscount() {
        ...
        const discountReq: DiscountReq = {
            ...
            type: DiscountTypeEnum.COURSE,
            courseId: this.courseId
        }

        if (this.mode === 'add') {
            this.discountService.createCourseDiscount(discountReq, this.courseId).subscribe({
                next: data => {
    ...
    redirectToDiscount() {
        this.router.navigate([`/instructor/courses/detail/${this.courseId}/discount`]);  // Chuyển hướng về trang discount
    }
